...
/// Loopback IPv4 address strictly listened on by the snapshot HTTP server (§7.1).
pub const SERVER_HOST: &str = "127.0.0.1";
/// Default port for snapshot HTTP server according to SPEC §7.1.
pub const DEFAULT_SERVER_PORT: u16 = 8765;
/// Endpoint path serving repository JSON snapshot.
pub const REPOSITORY_ENDPOINT: &str = "/repository.json";
/// Default timeout for polling during server shutdown checking.
pub const DEFAULT_POLL_TIMEOUT: Duration = Duration::from_millis(100);
/// Default timeout for HTTP socket read/write operations.
pub const DEFAULT_IO_TIMEOUT: Duration = Duration::from_secs(5);

/// Configuration options for the HTTP snapshot server (§7.1).
...
pub struct HttpServerConfig {
    ...
    pub port: u16,
    /// Poll timeout when checking for shutdown signals (default: 100ms).
    pub poll_timeout: Duration,
    /// Socket read and write timeout (default: 5s).
...
/// Serve a frozen in-memory repository snapshot over HTTP.
pub fn serve_repository(repo: &Repository, config: HttpServerConfig) -> Result<(), CliError> {
    ...
    let bind_host = &config.host;
    let bind_port = config.port;
    let listener = TcpListener::bind((bind_host.as_str(), bind_port))
        .map_err(|e| CliError::Custom(format!("failed to bind to {bind_host}:{bind_port}: {e}")))?;
    ...
    println!("http://{bind_host}:{actual_port}{REPOSITORY_ENDPOINT}");
    ...
    let poll_timeout_ms = config.poll_timeout.as_millis().min(i32::MAX as u128) as libc::c_int;
    ...
    while !SHUTDOWN.load(Ordering::SeqCst) {
        ...
        if (pfd.revents & libc::POLLIN) != 0 {
            match listener.accept() {
                Ok((mut stream, _)) => {
                    let _ = handle_connection(&mut stream, &snapshot_bytes, config.io_timeout);
                }
                Err(e)
...
mod tests {
    ...
    fn test_server_endpoint_routing() {
        ...
        let listener = TcpListener::bind((SERVER_HOST, 0)).unwrap();
        ...
        std::thread::spawn(move || {
            for _ in 0..4 {
                if let Ok((mut stream, _)) = listener.accept() {
                    let _ = handle_connection(&mut stream, &snapshot_bytes, DEFAULT_IO_TIMEOUT);
                }
            }
        });
        ...
        {
            let mut client = TcpStream::connect((SERVER_HOST, port)).unwrap();
            client
